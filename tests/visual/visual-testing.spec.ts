...
test.describe('Storybook Visual Testing', () => {
  test.beforeAll(async () => {
    ...
    storybookHelper = new StorybookHelper(process.env.STORYBOOK_URL || 'http://localhost:6006');
    ...
    test(`Visual test: ${story.title} - ${story.name}`, async ({ page, browserName }, testInfo) => {
      ...
      try {
        ...
        const screenshot = await storybookHelper.takeStoryScreenshot(page, {
          fullPage: false,
          mask: [
            // Mask any dynamic elements that shouldn't affect visual comparison
            '[data-testid="timestamp"]',
            '.sb-show-addons' // Hide Storybook addons panel if visible
          ]
        });
        ...
        if (!baselineExists) {
          ...
          testInfo.annotations.push({
            type: 'info',
            description: `Baseline snapshot created: ${snapshotKey}`
          });
        ...
        testInfo.annotations.push({
          type: 'error',
          description: `Visual test failed: ${error}`
        });
